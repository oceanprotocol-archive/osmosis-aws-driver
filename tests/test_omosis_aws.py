...
def test_complete():
    ...
    try:
        ...
        file_path = os.path.abspath('./LICENSE')
        # Upload a file
        s3_plugin.upload(file_path, f's3://{bucket_name}/test/LICENSE')
        ...
        assert cmp(file_path, '/tmp/test_osmosis_data_plugin_license')
    ...
    except (OsmosisError, Exception) as e:
        logger.error(f'There is problem: {e}')
        raise
    finally:
        if bucket_made:
            time.sleep(2)
